...
//        Glide.with(mContext).load(path).into(mImageView);
        Glide.with(mContext).load(path).placeholder(R.mipmap.ic_launcher).error(R.mipmap.ic_launcher).into(mImageView);
    }
    ...
    //设置下载优先级
    public static void load(Context mContext, byte[] path, ImageView mImageView) {
        Glide.with(mContext).load(path).priority(Priority.NORMAL).into(mImageView);
    }
